...
def im_detect_3d(im, net, rpn_conf, preprocess, p2, gpu=0, synced=False, return_base=False):
    ...
    """
    use_differentiable_nms   = False        if not ('use_nms_in_loss' in rpn_conf)             else rpn_conf.use_nms_in_loss
    diff_nms_pruning_method  = "linear"     if not('diff_nms_pruning_method' in rpn_conf)      else rpn_conf.diff_nms_pruning_method
    diff_nms_temperature     = 1            if not('diff_nms_temperature' in rpn_conf)         else rpn_conf.diff_nms_temperature
    diff_nms_valid_box_prob_threshold= 0.3  if not('diff_nms_valid_box_prob_threshold' in rpn_conf) else rpn_conf.diff_nms_valid_box_prob_threshold
    overlap_in_nms           = "2d"         if not ('overlap_in_nms' in rpn_conf)              else rpn_conf.overlap_in_nms
    diff_nms_group_boxes     = True         if not('diff_nms_group_boxes' in rpn_conf)         else rpn_conf.diff_nms_group_boxes
    diff_nms_mask_group_boxes= True         if not('diff_nms_mask_group_boxes' in rpn_conf)    else rpn_conf.diff_nms_mask_group_boxes
    diff_nms_group_size      = 100          if not('diff_nms_group_size' in rpn_conf)          else rpn_conf.diff_nms_group_size
    use_nms = True
    ...
    if return_base:
        ...
        cls, prob, bbox_2d, bbox_3d, feat_size, rois, acceptance_prob, acceptance_prob_cls = net(im)
    ...
    if ('orientation_bins' in rpn_conf) and rpn_conf.orientation_bins > 0:
        ...
        if ('has_un' in rpn_conf) and rpn_conf.has_un:
            bbox_un = bbox_3d[:, :, 10]
        if 'predict_acceptance_prob' in rpn_conf and rpn_conf.predict_acceptance_prob:
            bbox_un = acceptance_prob[:, :, 0]
    else:
    ...
    if not ('infer_2d_from_3d' in rpn_conf) or not (rpn_conf.infer_2d_from_3d):
        ...
        coords_3d_proj = p2_inv.dot(np.vstack((x3d_2d*z3d_2d, y3d_2d*z3d_2d, z3d_2d, np.ones(x3d_2d.shape))))
        x3d = coords_3d_proj[0]
        y3d = coords_3d_proj[1]
        z3d = coords_3d_proj[2]

        ry3d = convertAlpha2Rot(alp, z3d, x3d)
        coords_3d_raw = copy.deepcopy(coords_3d)
        coords_3d_raw[:, 0] = x3d
        coords_3d_raw[:, 1] = y3d
        coords_3d_raw[:, 2] = z3d
        coords_3d_raw[:, 6] = ry3d
    ...
    if ('has_un' in rpn_conf and rpn_conf.has_un) or ('predict_acceptance_prob' in rpn_conf and rpn_conf.predict_acceptance_prob):
        if 'use_acceptance_prob_for_nms' in rpn_conf and rpn_conf.use_acceptance_prob_for_nms:
            bbox_un = bbox_un[0].cpu().detach().numpy()
            scores = scores*bbox_un
    ...
    coords_3d_raw = coords_3d_raw[sorted_inds, :]
    cls_pred = cls_pred[sorted_inds]
    tracker = tracker[sorted_inds]

    if use_nms and synced and aboxes.shape[0] > 0:
    ...
    elif use_nms and aboxes.shape[0] > 0:
        ...
        coords_3d_raw = coords_3d_raw[0:min(rpn_conf.nms_topN_pre, coords_3d_raw.shape[0])]

        # nms
        if use_differentiable_nms:
            num_boxes = 500
            aboxes = aboxes[:500]
            ious_2d_for_nms_img = iou(aboxes[:, 0:4], aboxes[:, 0:4], mode='combinations')

            if overlap_in_nms == "2d":
                ious_for_nms_img    = ious_2d_for_nms_img

            else:
                coords_3d_raw = coords_3d_raw[:500]

                corners_3d_b1 = get_corners_of_cuboid(x3d= coords_3d_raw[:, 0],
                                              y3d= coords_3d_raw[:, 1],
                                              z3d= coords_3d_raw[:, 2],
                                              w3d= coords_3d_raw[:, 3],
                                              h3d= coords_3d_raw[:, 4],
                                              l3d= coords_3d_raw[:, 5],
                                              ry3d= coords_3d_raw[:, 6])
                corners_3d_b1 = torch.from_numpy(corners_3d_b1).float().cuda()
                ...
                ious_3d_for_nms_img     = ious_3d_for_nms_img.cpu().numpy()
                if overlap_in_nms == "3d":
                    ious_for_nms_img    = ious_3d_for_nms_img
                elif overlap_in_nms == "product":
                    ious_for_nms_img    = ious_2d_for_nms_img * ious_3d_for_nms_img

            keep_inds, _, scores_new = differentiable_nms(scores_unsorted= aboxes[:, 4], iou_unsorted= ious_for_nms_img, nms_threshold= rpn_conf.nms_thres, pruning_method= diff_nms_pruning_method, temperature= diff_nms_temperature, valid_box_prob_threshold = diff_nms_valid_box_prob_threshold, return_sorted_prob= False, group_boxes= diff_nms_group_boxes, mask_group_boxes= diff_nms_mask_group_boxes, group_size= diff_nms_group_size)
            keep_inds = keep_inds.numpy()
        else:
            num_boxes = aboxes.shape[0]
            # from lib.nms_others import navneeth_soft_nms
            # keep_inds     = navneeth_soft_nms(aboxes[:, 0:5].astype(np.float32), Nt= rpn_conf.nms_thres)

            # from lib.nms_others import distance_guided_nms
            # keep_inds, depths_new = distance_guided_nms(pred_bboxes= torch.FloatTensor(aboxes[:, 0:4]), pred_scores_new= torch.FloatTensor(aboxes[:, 4]), pred_deps= torch.FloatTensor(coords_3d_raw[:, 2]), threshold= rpn_conf.nms_thres)
            # keep_inds = np.array(keep_inds)
            # coords_3d_raw[:, 2] = depths_new.cpu().detach().numpy()

            # from lib.nms_others import dpp_nms
            # keep_inds = dpp_nms(torch.from_numpy(aboxes[:, 0:5].astype(np.float32)).cuda())

            keep_inds = gpu_nms(aboxes[:, 0:5].astype(np.float32), rpn_conf.nms_thres, device_id=gpu)
            keep_inds = np.array(keep_inds)

        # stack cls prediction
        aboxes = np.hstack((aboxes, cls_pred[:num_boxes, np.newaxis], coords_3d[:num_boxes], tracker[:num_boxes, np.newaxis]))
...
def get_nms(aboxes, conf, gpu= 0, score_threshold= None):
    # pre-nms
    aboxes = aboxes[0:min(conf.nms_topN_pre, aboxes.shape[0]), :]

    # NMS
    if aboxes.shape[0] > 0:
        # nms and suppress boxes
        keep_inds = gpu_nms(aboxes[:, 0:5].astype(np.float32), conf.nms_thres, device_id=gpu)
...
def test_kitti_3d_old(dataset_test, net, rpn_conf, results_path, test_path, has_rcnn=False):
    """
    ...
    for imind, impath in enumerate(imlist):
        ...
        # read in calib
        p2 = read_kitti_cal(os.path.join(test_path, dataset_test, split_name, 'calib', name + '.txt'))
        p2_inv = np.linalg.inv(p2)
        ...
        for boxind in range(0, min(rpn_conf.nms_topN_post, aboxes.shape[0])):
            ...
            if ('has_un' in rpn_conf and rpn_conf.has_un) or ('predict_acceptance_prob' in rpn_conf and rpn_conf.predict_acceptance_prob):
                un = box[13]
                if 'use_un_for_score' in rpn_conf and rpn_conf.use_un_for_score:
                    if 'use_acceptance_prob_for_nms' in rpn_conf and rpn_conf.use_acceptance_prob_for_nms:
                        # NMS scores are already multiplied by un. So skip multiplying again
                        pass
                    else:
                        score = score * un
    ...
    evaluate_kitti_results_verbose(data_folder= test_path, test_dataset_name= dataset_test, results_folder= results_path, split_name= split_name, test_iter= test_iter, conf= rpn_conf)

def convert_image_predictions_to_correct_entries(boxes_img_input, conf, p2):
    boxes_img = copy.deepcopy(boxes_img_input)
    ...
    # convert projections of 3D to True 3D
    p2_inv    = np.linalg.inv(p2)                                                                # 4 x 4
    coord3d   = backproject_2d_pixels_in_4D_format(p2_inv, boxes_img[:, 6:9].T, pad_ones= True)  # 4 x N
    coord3d   = coord3d.T                                                                        # N x 4
    ...
    # convert alpha into ry3d
    ry3d      = convertAlpha2Rot(alpha, z3d, x3d)
    ry3d      = snap_to_pi(ry3d)

    if ('has_un' in conf) and conf.has_un:
        un = score * boxes_img[:, 13]

    if ('has_un' in conf) and conf.has_un and (
            'use_un_for_score' in conf) and conf.use_un_for_score:
        score = un

    # Weight confidence by geometric confidence
    # h2d_general  = get_h2d_z3d_general(p2, x3d, y3d, z3d, l3d, w3d, h3d, rotY= ry3d)
    # conf_general = tranform_error_to_confidence(np.abs(h2d_general-height), temperature= 0.3)
    # score = 0.5*(score + (1.0 - conf_general))
    # score = 2 * score * (1.0 - conf_general) / (score + 1.0 - conf_general)

    # Update with the correct entries
    boxes_img[:, 4]  = score
    boxes_img[:, 6]  = x3d
    boxes_img[:, 7]  = y3d
    boxes_img[:, 8]  = z3d
    ...
    # Save projections of the 3d centers as well
    centers_3d_2d = np.hstack((x3d_2d[:, np.newaxis], y3d_2d[:, np.newaxis], z3d_2d[:, np.newaxis]))
    boxes_img     = np.hstack((boxes_img, centers_3d_2d))

    return boxes_img


def write_image_boxes_to_txt_file(boxes_img, conf, save_folder, id, convention="kitti", write= True, save_constraint= False, precision= 6):
    """
        Creates a prediction file for each of the images
    """
    if convention == "kitti":
        file_path     = os.path.join(save_folder, id + '.txt')
        #======================================================================
        # Do NOT change the precision from six
        #
        # Abhinav: Reducing the precision unfortunately makes the KITTI
        # evaluation binary behave abnormally and the mAP results are reported
        # less than expected
        #======================================================================
        text_to_write = get_text_to_write_in_kitti_format(boxes_img, conf, convention= convention, save_constraint= save_constraint, precision= 6)
...
def get_text_to_write_in_kitti_format(boxes_img, conf, convention="kitti", save_constraint= False, precision= 6):
    precision_format = '{:.' + str(int(precision)) + 'f}'

    N_boxes= boxes_img.shape[0]
    ...
    if save_constraint:
        shift_index = 17
        w2d         = boxes_img[:, shift_index]
        h2d         = boxes_img[:, shift_index+1]
        h2d_general = boxes_img[:, shift_index+2]
        h2d_special = boxes_img[:, shift_index+3]

    text_to_write = ""
    if convention == "kitti":
        for i in range(N_boxes):
            cls = conf.lbls[cls_ind[i]]
            ...
            Each row = (cls, -1, -1, alpha, x1, y1, x2, y2, h3d, w3d, l3d, x3d, y3d, z3d, ry3d, score)
            '''
            #if cls == 'Car':
            text_to_write += ('{} -1 -1' + (' ' + precision_format)*13).format(cls, alpha[i], x1[i], y1[i], x2[i], y2[i], h3d[i], w3d[i],
                                                          l3d[i], x3d[i], y3d[i], z3d[i], ry3d[i], score[i])
            if save_constraint:
                text_to_write += ( (' ' + precision_format)*4).format(w2d[i], h2d[i], h2d_general[i], h2d_special[i])
            text_to_write += '\n'
    return text_to_write
...
def evaluate_kitti_results_verbose(data_folder, test_dataset_name, results_folder, split_name= "validation", test_iter= None, conf= None, use_logging=True, fast=False, default_eval='evaluate_object'):
    """
    ...
    gt_folder         = os.path.join(data_folder, test_dataset_name, split_name, "label_2")
    # test_path contains data at the end
    # replace by splits_prep
    devkit_folder     = data_folder#.replace("data", "splits_prep")
    ...
    results_obj.main = run_kitti_eval_script(eval_binary_path, results_data= stats_save_folder, gt_folder= gt_folder, lbls= conf.lbls, use_40=True)
    print("")
    if use_logging:
        logging.info("Running for thresholds [0.7, 0.5, 0.5]...")
    # print main experimental results for each class, and each task
    for lbl in conf.lbls:
    ...
    results_obj.side = run_kitti_eval_script(eval_binary_path, results_data= stats_save_folder, gt_folder= gt_folder, lbls= conf.lbls, use_40=True)
    print("")
    if use_logging:
        logging.info("Running for thresholds [0.5, 0.3, 0.3]...")
    # print main experimental results for each class, and each task
    for lbl in conf.lbls:
    ...
    if fast or ('fast_eval' in conf and conf.fast_eval): return
    ...
    for k, dis_key in enumerate(dis_keys):
        ...
        for iou_key in iou_keys:
            ...
            #eval_binary_path      = os.path.join("/home/abhinav/project/mono_object_detection_July_09/splits_prep/kitti_split1/devkit/cpp", eval_key)
            tmp_obj               = run_kitti_eval_script(eval_binary_path, results_data= stats_save_folder, gt_folder= gt_folder, lbls= conf.lbls, use_40= True)
    ...
    try:
        ...
        save_kitti_ROC(results_obj, stats_save_folder, conf.lbls)
