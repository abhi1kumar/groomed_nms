...
"""
    Sample Run:
    python plot/plot_prediction_with_ground_truth.py -i output/dummy_kitti/results/results_test
    python plot/plot_prediction_with_ground_truth.py -t 0.7
    python plot/plot_prediction_with_ground_truth.py -i output/kitti_3d_warmup/results/results_test/ output/kitti_3d_uncertainty/results/results_test output/groumd_nms/results/results_test --labels Warmup Kinematic\ \(Image\) GrooMeD-NMS
    python plot/plot_prediction_with_ground_truth.py -i output/M3D-RPN/results/results_test/ output/kitti_3d_uncertainty/results/results_test output/Kinematic_video/results/results_test/ output/groumd_nms/results/results_test --labels M3D-RPN Kinematic\ \(Image\) Kinematic\ \(Video\) GrooMeD-NMS
    python plot/plot_prediction_with_ground_truth.py -i output/accept_prob_dynamic/results/results_test_with_pred/ output/groumd_nms/results/results_with_pred/ output/accept_prob_dynamic/results/results_test/ output/groumd_nms/results/results_test --labels W/o\ NMS\(Pred\) With\ NMS\(Pred\) W/o\ NMS\(Pred*Class\) With\ NMS\(Pred*Class\)

    Plots
    - which combination of confidences correlate with error in z and conf in z
    - to check if there is constant scale issue
    - predictions vs ground truth of different parameters
    - correlations between diff errors and error in z (this can be absolute or relative)
    - Saves the error as a pkl file which can be used later
"""
import os, sys
...
from lib.core import iou, iou3d
from lib.groomed_nms import *
from lib.math_3d import *
from lib.file_io import read_csv, pickle_read, pickle_write
...
for i in range(num_input_folders):
    for prediction_folder_relative in prediction_folders_relative:
        ...
        if os.path.exists(full_folder_path):
            big_list = read_folder_and_get_all_errors(curr_folder, prediction_folder_relative, num_images= num_images, num_predictions_boxes= num_predictions_boxes, threshold_score= threshold_score, threshold_depth= threshold_depth)
            iou_2d_all = big_list[3]
            score      = big_list[2]
            plot_one_error_with_box_error(score, iou_2d_all, name=r"Box Confidence", plt= plt, show_message= show_message, throw_samples_flag= throw_samples_flag, throw_samples_in_bin= throw_samples_in_bin, iou_on_x= iou_on_x, do_decoration= False, label= labels[i], color= colors_list[i], bins= bins)
        else:
            print("{} DNE".format(full_folder_path))
...
xticklabels = [str(round(float(x),1)) if i%2 == 0 else "" for i,x in enumerate(xticks.tolist()) ]
plt.xticks(xticks, xticklabels)
plt.yticks(np.arange(ymin2d, 1 + ydelta, step=2*ystep))
plt.xlim((0, 1.0))
plt.ylim(bottom= ymin2d, top= 1 + ydelta)
plt.xlabel(r"Box IoU$_{2D}$")
plt.ylabel("Box Confidence")
plt.legend(loc= "lower right", fontsize= params.legend_fs+4, borderaxespad= params.legend_border_axes_plot_border_pad, labelspacing= params.legend_vertical_label_spacing)#, borderpad= params.legend_border_pad, handletextpad= params.legend_marker_text_spacing+1)
save_file = "scores_vs_iou2d.png"
if args.save_prefix is not None:
    save_file = save_file.split(".")[0] + "_"  + args.save_prefix + ".png"
save_path = os.path.join(params.IMAGE_DIR, save_file)
print("")
