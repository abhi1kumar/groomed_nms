...
def Config():
    ...
    conf.pretrained = 'output/kitti_3d_warmup_split2/weights/model_80000_pkl'
    ...
    conf.predict_acceptance_prob = True
    conf.acceptance_prob_lambda  = 0
    conf.use_nms_in_loss = True
    conf.diff_nms_temperature = 0.1
    conf.diff_nms_pruning_method = "linear"
    conf.diff_nms_boxes_2d = "normal"
    conf.diff_nms_valid_box_threshold = 0.3
    conf.diff_nms_group_boxes = True
    conf.diff_nms_mask_group_boxes = True
    conf.diff_nms_group_size = 100
    conf.after_nms_lambda = 0.05
    conf.after_nms_loss_mode = "rank"
    conf.best_target_box_beta = 0.3
    ...
    conf.use_acceptance_prob_for_nms = True

    conf.boxes_for_acceptance_prob = "foregrounds"
    conf.acceptance_prob_mode  = "likelihood"
    conf.use_acceptance_prob_in_regression_loss = True
    conf.weigh_acceptance_prob_regularization = True
    conf.rank_with_class_confidence = False
    conf.rank_boxes_of_all_images_at_once = False

    conf.infer_2d_from_3d = False
    conf.score_thres = 0.6
    ...
    # datasets
    conf.dataset_test = 'kitti_split2'
    conf.datasets_train = [{'name': 'kitti_split2', 'anno_fmt': 'kitti_det', 'im_ext': '.png', 'scale': 1}]
